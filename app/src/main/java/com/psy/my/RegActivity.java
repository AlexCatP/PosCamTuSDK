...
//            if(!edVerify.getText().toString().matches("[0-9]{6}")){msgVerify.setText("验证码只能是6位数字");isConnectWS=false;}
//            else
//            {
//                if(!verify.equals(edVerify.getText().toString())){msgVerify.setText("验证码错误");isConnectWS=false;}
//                else msgVerify.setText("");
//            }
            if(docGray.getVisibility()==View.VISIBLE){Common.display(getApplicationContext(),"请阅读并同意注册协议");isConnectWS=false;}
