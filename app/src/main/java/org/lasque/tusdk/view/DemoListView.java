 ...
 * @author Clear
 */
public class DemoListView extends TuGroupListView<Base, DemoListCell, GroupHeader, DemoListHeader>
{
	/** 范例列表行点击动作 */
	...
	public interface DemoListViewDelegate
	{
		 ...
		 *            范例列表行点击动作
		 */
		void onDemoListViewSelected(DemoListView view, Base simple, DemoListItemAction action);
	}
	...
	/** 加载范例列表 */
	public void loadSimples(Group group)
	{
		this.setDataSource(new SimpleGroupDataSource(group));
	}

	/** 选中一个示例 */
	private void onSelectedItem(Base itemData)
	{
	...
	/** 范例列表行点击动作 */
	private class DemoListViewItemClick implements GroupListViewItemClickListener<Base, DemoListCell>
	{
		@Override
		public void onGroupItemClick(Base itemData, DemoListCell itemView, TuSdkIndexPath indexPath)
		{
	...
	private class SimpleGroupDataSource implements TuSdkDataSource
	{
		 ...
		 *            范例分组
		 */
		public SimpleGroupDataSource(Group group)
		{
			this.splitDatas(group);
		}

		/** 分组数据 */
		private void splitDatas(Group group)
		{
		...
		public void onViewBinded(TuSdkIndexPath indexPath, View view)
		{
			...
			{
				((DemoListCell) view).setModel((Base) mode);
			}
